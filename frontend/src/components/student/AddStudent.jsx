...
const AddStudent = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        const { regNo, department } = formData;

        // Validate Register Number and Department
        if (!regNo || regNo.length !== 10) {
            alert("Register number must be exactly 10 digits.");
            return;
        }

        const departmentData = departments.find(dep => dep._id === department);
        if (!departmentData) {
            alert("Selected department is invalid.");
            return;
        }

        const depIdFromRegNo = regNo.slice(6, 8); // Extract 6th and 7th characters
        if (depIdFromRegNo !== departmentData.dep_id) {
            alert(`Register number's 6th and 7th characters (${depIdFromRegNo}) do not match the selected department's ID (${departmentData.dep_id}).`);
            return;
        }
